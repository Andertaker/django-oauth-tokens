...
class InstagramAuthRequest(AuthRequestBase):
    ...
    '''
    provider = 'instagram'
    form_action_domain = 'https://instagram.com'
    ...
    headers = {
        ...
        'Referer': 'https://api.instagram.com/oauth/authorize?oauth_token=prWUDG4s5HpwEnPY3Pun1CDgsXSVlfIU',
...
class InstagramAccessToken(AccessTokenBase):
    ...
    def get(self):
        oauth_token = super(InstagramAccessToken, self).get()
    ...
    def authorization_get_request(self):
        ...
        return self.auth_request.session.get(url=authorization_url)  # instagram don't like headers here
    ...
    def process_authorization_response(self, response):
        ...
        log.debug('Got instagram verifier: %s for user %s' % (code, self.auth_request.username))
        return str(code)
